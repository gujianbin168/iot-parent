...
public class BcryptUtils {
    ...
    public static String key = "XDMGStGtOpDRxNyvYt54EQ==";
    ...
    public static Boolean decoder(String unencoder,String encoder) {
        return passwordEncoder.matches(unencoder,encoder);
    }
...
//            md5Hash = EncryptUtil.MD5Hash(temp, 16, temp.length - 16);
            ...
            md5Hasn = BcryptUtils.MD5Hash(encrypt, 0, encrypt.length);
        } catch (Exception e) {
            e.printStackTrace();
        }
        // 组合消息体
        byte[] totalByte = BcryptUtils.addMD5(md5Hasn, encrypt);
        
        // 取密钥和偏转向量
        byte[] key = new byte[8];
        byte[] iv = new byte[8];
        getKeyIV(BcryptUtils.key, key, iv);
        ...
        byte[] temp = null;
        try {
            temp = BcryptUtils.DES_CBC_Encrypt(totalByte, deskey, ivParam);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static String decrypt(String xmlStr) throws Exception {
        ...
        byte[] temp = null;
        try {
            temp = BcryptUtils.DES_CBC_Decrypt(encBuf, deskey, ivParam);
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        byte[] md5Hash = null;
        try {
            md5Hash = BcryptUtils.MD5Hash(temp, 16, temp.length - 16);
        } catch (Exception e) {
            e.printStackTrace();
        }
...
/*	public static void main(String[] args) throws Exception {
		...
		System.out.println(EncryptUtil.encrypt(password, "abc"));
		System.out.println(EncryptUtil.encrypt(password, "abc"));
		//System.out.println(EncryptUtil.decrypt("3a5b93e521b63e19c16f4f7f02a9fb2b06796b1a"));
		
		System.out.println(BcryptUtils.encoder(password));
		System.out.println(BcryptUtils.encoder(password));
		System.out.println(BcryptUtils.decoder(password,"$2a$10$eItK5okiiy/gRxhhPe3rWuLqNKAgk5BKHpnw5QCuzL25Rjq8uVCnm"));
		System.out.println(BcryptUtils.decoder(password,"$2a$10$HonXycoYRSCIYHzcs/uQBeFcXNBcbJuuF86JNeFpR0mvHAT9mQoQW"));
		
		
		System.out.println(BcryptUtils.decrypt("oqGiG3w2C/s4l945xI++My4Wpv2cCyLi"));
	}*/
}
